...
@RestController
@RequestMapping("/api/v1/ccDistributors")
public class  CcDistributorController {

    @Autowired
    private CcDistributorService ccDistributorService;
    ...
    public  Payload findPage(CcDistributor eo,
        ...
        return new Payload(ccDistributorService.findPage(eo, page, size));
    }

    @GetMapping("/list")
    //@ApiOperation(value = "树形查询", notes = "查询全部请求")
    public Payload findAll(CcDistributor eo) {
        return new Payload(ccDistributorService.findAll(eo));
    }

    @GetMapping("/{id}")
    public Payload detail(@PathVariable(value = "id", required = true) Integer  pk) {
        return new Payload(ccDistributorService.detail(pk));
    }
...
//@ApiOperation(value = "根据id修改", notes = "根据id修改CcDistributor")
    public Payload update(@PathVariable(value = "id", required = true) Integer  pk, @RequestBody CcDistributor eo) {
     eo.setId(pk);
     return new Payload(ccDistributorService.update(pk, eo));
    }

    @PostMapping
    //@ApiOperation(value = "创建CcDistributor", notes = "创建CcDistributor")
    public Payload create(@RequestBody CcDistributor eo) {
        return new Payload(ccDistributorService.create(eo));
    }
...
//@ApiOperation(value = "根据id删除CcDistributor", notes = "根据id删除CcDistributor")
    public Payload delete(@PathVariable(value = "id", required = true) Integer  pk) {
        return new Payload(ccDistributorService.delete(pk));
    }
    ...
    public Payload delete(@RequestParam(required = true) Integer [] ids) {
        return new Payload(ccDistributorService.delete(ids));
    }
