...
public class  CcBankAccountController {

    @Autowired
    private CcBankAccountService ccBankAccountService;
    ...
    public  Payload findPage(CcBankAccount eo,
        ...
        return new Payload(ccBankAccountService.findPage(eo, page, size));
    }

    @GetMapping("/list")
    //@ApiOperation(value = "树形查询", notes = "查询全部请求")
    public Payload findAll(CcBankAccount eo) {
        return new Payload(ccBankAccountService.findAll(eo));
    }

    @GetMapping("/{id}")
    public Payload detail(@PathVariable(value = "id", required = true) Integer  pk) {
        return new Payload(ccBankAccountService.detail(pk));
    }
...
//@ApiOperation(value = "根据id删除CcBankAccount", notes = "根据id删除CcBankAccount")
    public Payload delete(@PathVariable(value = "id", required = true) Integer  pk) {
        return new Payload(ccBankAccountService.delete(pk));
    }
    ...
    public Payload delete(@RequestParam(required = true) Integer [] ids) {
        return new Payload(ccBankAccountService.delete(ids));
    }
