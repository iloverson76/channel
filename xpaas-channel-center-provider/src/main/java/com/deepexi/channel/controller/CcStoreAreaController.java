...
public class  CcStoreAreaController {

    @Autowired
    private CcStoreAreaService ccStoreAreaService;
    ...
    public  Payload findPage(CcStoreArea eo,
        ...
        return new Payload(ccStoreAreaService.findPage(eo, page, size));
    }

    @GetMapping("/list")
    //@ApiOperation(value = "树形查询", notes = "查询全部请求")
    public Payload findAll(CcStoreArea eo) {
        return new Payload(ccStoreAreaService.findAll(eo));
    }

    @GetMapping("/{id}")
    public Payload detail(@PathVariable(value = "id", required = true) Integer  pk) {
        return new Payload(ccStoreAreaService.detail(pk));
    }
...
//@ApiOperation(value = "根据id删除CcStoreArea", notes = "根据id删除CcStoreArea")
    public Payload delete(@PathVariable(value = "id", required = true) Integer  pk) {
        return new Payload(ccStoreAreaService.delete(pk));
    }
    ...
    public Payload delete(@RequestParam(required = true) Integer [] ids) {
        return new Payload(ccStoreAreaService.delete(ids));
    }
