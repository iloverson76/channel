...
public class  CcStoreTypeController {

    @Autowired
    private CcStoreTypeService ccStoreTypeService;
    ...
    public  Payload findPage(CcStoreType eo,
        ...
        return new Payload(ccStoreTypeService.findPage(eo, page, size));
    }

    @GetMapping("/list")
    //@ApiOperation(value = "树形查询", notes = "查询全部请求")
    public Payload findAll(CcStoreType eo) {
        return new Payload(ccStoreTypeService.findAll(eo));
    }

    @GetMapping("/{id}")
    public Payload detail(@PathVariable(value = "id", required = true) Integer  pk) {
        return new Payload(ccStoreTypeService.detail(pk));
    }
...
//@ApiOperation(value = "根据id删除CcStoreType", notes = "根据id删除CcStoreType")
    public Payload delete(@PathVariable(value = "id", required = true) Integer  pk) {
        return new Payload(ccStoreTypeService.delete(pk));
    }
    ...
    public Payload delete(@RequestParam(required = true) Integer [] ids) {
        return new Payload(ccStoreTypeService.delete(ids));
    }
