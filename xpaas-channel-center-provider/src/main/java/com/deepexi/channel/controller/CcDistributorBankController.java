...
public class  CcDistributorBankController {

    @Autowired
    private CcDistributorBankService ccDistributorBankService;
    ...
    public  Payload findPage(CcDistributorBank eo,
        ...
        return new Payload(ccDistributorBankService.findPage(eo, page, size));
    }

    @GetMapping("/list")
    //@ApiOperation(value = "树形查询", notes = "查询全部请求")
    public Payload findAll(CcDistributorBank eo) {
        return new Payload(ccDistributorBankService.findAll(eo));
    }

    @GetMapping("/{id}")
    public Payload detail(@PathVariable(value = "id", required = true) Integer  pk) {
        return new Payload(ccDistributorBankService.detail(pk));
    }
...
//@ApiOperation(value = "根据id删除CcDistributorBank", notes = "根据id删除CcDistributorBank")
    public Payload delete(@PathVariable(value = "id", required = true) Integer  pk) {
        return new Payload(ccDistributorBankService.delete(pk));
    }
    ...
    public Payload delete(@RequestParam(required = true) Integer [] ids) {
        return new Payload(ccDistributorBankService.delete(ids));
    }
